...
class Spectrometer():
    ...
    def get_spectrum(self, fname=None):
        ...
        ----------
        fname : str, optional
            File name to save the measured spectrum to (in a netCDF format). If
            None then the spectrum is not saved.

        Returns
        -------
        xr.DataArray
            Contains the spectrum wavelengths, intensities and metadata
        """
        # Get the wavelengths
        ...
        # If give a file name, save the spectrum
        if fname is not None:
            spectrum.to_netcdf(fname)
